...
3. Create or resume `candidate/<slug>` from each affected child repository's
   ...
   next submission.
4. **Re-derive before patching edge cases.** Before each new edge-case fix,
   restate the required invariant without the current mechanism. Stop
   automatically when one mechanism reaches three distinct failure modes, its
   validation centers on those failures, or the next fix adds an exception,
   owner, handoff, fallback, or misplaced policy. Re-derive the smallest design
   from the invariant and domain documentation; compare materially different
   options, including deletion or deepening an existing owner. Retain evidence
   when this changes direction. Resume when documentation decides the choice;
   otherwise route the semantic decision to the operator using step 2.
5. Implement findings whose disposition is `repair` without narrowing the task
...
6. Before submission, validate the implementation and inspect the complete diff
   against the task for obvious mistakes. Do not run the formal independent
   review before the exact candidate is recorded.
7. Submit exact remote candidates in dependency order without releasing the claim:
